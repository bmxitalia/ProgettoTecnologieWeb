...
<html>
	<head>
		...
		<title>Aggiorna password admin | Padova Marathon</title>
	...
	<header>
		...
		<ul class="breadcrumb">
			...
			<li><a title="Ti trovi sulla pagina Modifica password" tabindex="9">password</a></li>
		</ul>
		<!-- end breadcrumb -->
	</header>
	<body>
		...
		<div class="container" id="body">
			<div class="rounded-container">
				<h1 class="title" tabindex="10">Aggiorna Password</h1>
				<?php
					$dbAccess=new dbAccess();
					$dbAccess->configure();
					$tabI=$dbAccess->createUpdatePwdForm();
				?>
			</div>
		</div>
		<!-- end container -->
		<?php
			if(isset($_POST['submit'])){
				$vecchiaInserita=$_POST['password1'];
				$nuova=$_POST['password2'];
				$nuova2=$_POST['password3'];
				$vecchia=$dbAccess->getPwd();
				if($vecchia==$vecchiaInserita){
					if($nuova!=$vecchiaInserita){
						if($nuova==$nuova2){
							$query="update admin set password='".$nuova."' where password='".$vecchia."';";
							...
							echo "<div class='alert alert-danger'>
									...
									<p  tabindex='0'>Le nuove password inserite non corrispondono</p>
						...
						echo "<div class='alert alert-danger'>
								<h3  tabindex='0'>Attenzione!</h3><br/>
								<p tabindex='0'>La nuova password non può coincidere con la vecchia!</p>
								<a href='pannelloAdmin.php'>Torna alla Home</a>
							</div>";
					}
				}else{
					echo "<div class='alert alert-danger'>
							<h3 tabindex='0'>Attenzione!</h3><br/>
							<p tabindex='0'>La vecchia password non è quella corretta!</p>
							<a href='pannelloAdmin.php'>Torna alla Home</a>
						</div>";
				}
			}
